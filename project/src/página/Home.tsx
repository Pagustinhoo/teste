...
// IMÓVEIS EM DESTAQUE (na ordem que você pediu: 10, 2, 3, 4, 5)
const featuredProperties = [10, 2, 3, 4, 5]
  ...
  .map((property: Property) => {
    ...
    // Proteção para nunca mostrar "aluguel" ou "venda" como título
    const tituloExibido = (() => {
      const tipoLower = property.tipo.trim().toLowerCase();
      if (tipoLower === 'aluguel' || tipoLower === 'venda' || !property.tipo) {
        const cidade = property.endereco.split('-').pop()?.trim() || 'Cidade';
        return isAluguel ? `Imóvel para aluguel em ${cidade}` : `Imóvel para venda em ${cidade}`;
      }
...
const Home: React.FC = () => {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* HERO SECTION — COM IMAGEM DE FUNDO VOLTOU! */}
      <section className="relative bg-gradient-to-r from-blue-600 via-purple-600 to-blue-800 text-white overflow-hidden">
        ...
        {/* IMAGEM DE FUNDO QUE VOCÊ AMAVA */}
        <div
          className="absolute inset-0 bg-cover bg-center opacity-40"
          style={{
            backgroundImage:
              'ur[](https://images.pexels.com/photos/106399/pexels-photo-106399.jpeg?auto=compress&cs=tinysrgb&w=1920)',
          }}
        ></div>
          ...
          <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl p-6">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <Link
                ...
                className="bg-blue-600 text-white px-8 py-3 rounded-lg hover:bg-blue-700 flex items-center justify-center gap-2 font-semibold transition"
              >
                <Search size={20} />
                Buscar
              </Link>
