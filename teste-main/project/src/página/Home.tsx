...
const Home: React.FC = () => {
  ...
  // IMÓVEL FIXO EM DESTAQUE: ID 10 (Sobrado 3 quartos Los Angeles - Maringá)
  const priorityProperty = properties.find(p => p.id === 10)!;

  // Outros 5 imóveis em destaque (excluindo o ID 10 para não repetir)
  const otherFeatured = properties
    .filter(p => p.id !== 10)
    .slice(0, 5);

  // Junta: primeiro o ID 10, depois os outros 5
  const featuredProperties = [priorityProperty, ...otherFeatured].map((property: Property) => ({
    id: property.id,
    title: property.tipo, // Agora usa exatamente como está escrito no banco (com capitalização correta)
    ...
    <div className="min-h-screen">
      ...
      <section className="relative bg-gradient-to-r from-blue-600 via-purple-600 to-blue-800 text-white">
        ...
        <div
          ...
          style={{
            backgroundImage:
              'ur[](https://images.pexels.com/photos/280222/pexels-photo-280222.jpeg?auto=compress&cs=tinysrgb&w=1920)'
          }}
        ></div>
          ...
          <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl p-6">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <Link
                ...
                className="bg-blue-600 text-white px-8 py-3 rounded-lg hover:bg-blue-700 flex items-center gap-2 font-semibold transition"
              >
                <Search size={20} />
                Buscar
              </Link>
