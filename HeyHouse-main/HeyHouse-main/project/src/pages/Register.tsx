...
const USERS_KEY = 'heyhouse_users';
const CURRENT_USER_KEY = 'heyhouse_user';
...
const registerLocal = async (data: any) => {
  // simula requisição/latência
  return new Promise<{ success: boolean; error?: string }>((resolve) => {
    setTimeout(() => {
      const users = loadUsers();
      const exists = users.find((u: any) => u.email === data.email);
      if (exists) return resolve({ success: false, error: 'Email já cadastrado' });
      ...
      users.push(newUser);
      saveUsers(users);
      try { localStorage.setItem(CURRENT_USER_KEY, JSON.stringify(newUser)); } catch {}
...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    const result = await registerLocal(formData);
    setIsLoading(false);
