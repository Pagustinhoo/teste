...
const propertiesList: Property[] = Array.isArray(_raw) ? _raw : Array.isArray((propertiesModule as any).properties) ? (propertiesModule as any).properties : [];
const agentsList: Agent[] = Array.isArray((propertiesModule as any).agents) ? (propertiesModule as any).agents : [];

const getPropertyById = (id: number | string): Property | null => {
...
const getAgentById = (id: number | string): Agent | null => {
...
const PropertyDetail: React.FC = () => {
  ...
  // Corrigir busca por id string ou number
  const property = getPropertyById(id ?? '1');
  const agent = property ? getAgentById((property as any).agentId ?? (property as any).agent?.id ?? '1') : null;

  // Garantir estruturas mínimas para renderização
  ...
  if ((!safeProperty.images || safeProperty.images.length === 0) && safeProperty.image) {
    safeProperty.images = [safeProperty.image];
  }
  if (!safeProperty.images || safeProperty.images.length === 0) {
    safeProperty.images = ['/placeholder.jpg'];
  }
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
              <div className="relative">
                <img
                  src={safeProperty.images![currentImageIndex] ?? '/placeholder.jpg'}
                  alt={safeProperty.title ?? 'Imóvel'}
                ...
                <div className="absolute top-4 left-4">
                  <span className={`px-3 py-1 rounded-full text-sm font-semibold ${safeProperty.type === 'venda' ? 'bg-green-500 text-white' : 'bg-blue-500 text-white'}`}>
                    {safeProperty.type === 'venda' ? 'Venda' : 'Aluguel'}
                  </span>
                </div>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
              <div className="flex items-start justify-between mb-6">
                <div>
                  <h1 className="text-3xl font-bold text-gray-800 mb-2">{safeProperty.title}</h1>
                  <div className="flex items-center text-gray-600 mb-4">
                    <MapPin size={20} className="mr-2" />
                    <span className="text-lg">{safeProperty.location}</span>
                  </div>
              ...
              <div className="mb-8">
                <div className="text-4xl font-bold text-blue-600 mb-2">
                  R$ {safeProperty.price?.toLocaleString('pt-BR')}
                  {safeProperty.type === 'aluguel' && <span className="text-lg text-gray-500">/mês</span>}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="font-semibold">{safeProperty.bedrooms ?? 'N/A'}</div>
                  <div className="text-sm text-gray-600">Quartos</div>
                </div>
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <Bath size={24} className="mx-auto mb-2 text-blue-600" />
                  <div className="font-semibold">{safeProperty.bathrooms ?? 'N/A'}</div>
                  <div className="text-sm text-gray-600">Banheiros</div>
                </div>
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <Square size={24} className="mx-auto mb-2 text-blue-600" />
                  <div className="font-semibold">{safeProperty.area ?? 'N/A'}m²</div>
                  <div className="text-sm text-gray-600">Área</div>
                </div>
                <div className="text-center p-4 bg-gray-50 rounded-lg">
                  <Car size={24} className="mx-auto mb-2 text-blue-600" />
                  <div className="font-semibold">{safeProperty.parking ?? 0}</div>
              ...
              <div className="mb-8">
                ...
                <p className="text-gray-600 leading-relaxed">{safeProperty.description}</p>
              </div>
              ...
              <div className="text-center mb-6">
                ...
                <div className="flex items-center justify-center space-x-4 mt-3">
                  <div className="flex items-center space-x-1">
                    ...
                    <span className="text-sm font-semibold">{safeAgent.rating ?? '-'}</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <Award size={16} className="text-blue-600" />
                    <span className="text-sm">{safeAgent.experience ?? 0} anos</span>
                  </div>
                </div>
                ...
                <div className="mt-3">
                  <div className="flex flex-wrap gap-1 justify-center">
                    {(safeAgent.specialties ?? []).map((specialty, index) => (
