...
const Login: React.FC = () => {
  ...
  const fakeLogin = async (email: string, password: string) => {
    // Simula chamada assíncrona de login (substituir por API real se necessário)
    return new Promise<{ success: boolean; error?: string }>((resolve) => {
      setTimeout(() => {
        const match = demoAccounts.find(a => a.email === email && a.password === password);
        if (match) return resolve({ success: true });
        // aceitar qualquer email válido e senha >= 6 como "sucesso" (ajuste conforme necessidade)
        if (email.includes('@') && password.length >= 6) return resolve({ success: true });
